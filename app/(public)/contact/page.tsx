import type { Metadata } from "next";
import ContactForm from "@/components/public/ContactForm";
import { breadcrumbJsonLd } from "@/lib/seo";

export const metadata: Metadata = {
  title: "Contact Us",
  description:
    "Get in touch with Evercool Thailand. Call, WhatsApp, or send a message. Serving all of Thailand.",
};

export default function ContactPage() {
  return (
    <main className="page-enter px-4 md:px-10 pt-6 pb-8">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-ec-text">Contact Us</h1>
        <p className="text-sm text-ec-text-muted mt-1">
          We&apos;re here to help with all your HVAC &amp; IAQ needs
        </p>
      </div>

      {/* Quick Contact Cards */}
      <div className="flex flex-col gap-3 mb-6">
        <a
          href="tel:+66955622892"
          className="flex items-center gap-4 bg-ec-card rounded-2xl border border-ec-border p-4 hover:border-ec-teal/30 transition-all active:scale-[0.98]"
        >
          <div className="w-10 h-10 rounded-full bg-ec-teal/10 flex items-center justify-center shrink-0">
            <svg className="w-5 h-5 text-ec-teal" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
            </svg>
          </div>
          <div>
            <p className="text-xs font-semibold text-ec-text-muted uppercase tracking-wide">Call Us</p>
            <p className="text-sm font-bold text-ec-text mt-0.5">+66 95-562-2892</p>
          </div>
        </a>

        <a
          href="https://wa.me/66955622892"
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-4 bg-ec-card rounded-2xl border border-ec-border p-4 hover:border-[#25D366]/40 transition-all active:scale-[0.98]"
        >
          <div className="w-10 h-10 rounded-full bg-[#25D366]/10 flex items-center justify-center shrink-0">
            <svg className="w-5 h-5 text-[#25D366]" viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
          </div>
          <div>
            <p className="text-xs font-semibold text-ec-text-muted uppercase tracking-wide">WhatsApp</p>
            <p className="text-sm font-bold text-ec-text mt-0.5">Chat with our team</p>
          </div>
        </a>

        <div className="bg-ec-card rounded-2xl border border-ec-border p-4">
          <p className="text-xs font-semibold text-ec-text-muted uppercase tracking-wide mb-1">Hours</p>
          <p className="text-sm font-semibold text-ec-text">Mon to Sat</p>
          <p className="text-xs text-ec-text-muted">8:00 to 18:00</p>
        </div>

        {/* Address */}
        <div className="bg-ec-card rounded-2xl border border-ec-border p-4">
          <p className="text-xs font-semibold text-ec-text-muted uppercase tracking-wide mb-1">Office Address</p>
          <p className="text-sm font-semibold text-ec-text">383 (3rd Floor) Bond Street Road</p>
          <p className="text-xs text-ec-text-muted">Bangphut, Pakkret, Nonthaburi 11120</p>
          <p className="text-xs text-ec-text-muted">Thailand</p>
          <a
            href="https://maps.google.com/?q=383+Bond+Street+Road+Bangphut+Pakkret+Nonthaburi+Thailand"
            target="_blank"
            rel="noopener noreferrer"
            className="text-xs text-ec-teal hover:underline mt-2 inline-block"
          >
            Open in Google Maps
          </a>
        </div>
      </div>

      {/* Contact Form */}
      <ContactForm />

      {/* Schema */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(
            breadcrumbJsonLd([
              { name: "Home", url: "https://evercoolthailand.com" },
              { name: "Contact", url: "https://evercoolthailand.com/contact" },
            ])
          ),
        }}
      />
    </main>
  );
}
